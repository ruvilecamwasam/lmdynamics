...
ss = getSteadyStates();
y0 = [ss.xs+0.1; 0; ss.as+0.01];
[t,y] = simEOM(tRange,y0,@idealEOM,@plotTimeDomainDuring);

plotWavelet(t,y);

%% Simulation parameters %%
function params = getParams()
    params = struct('g',0.5,'Delta0',0,'eta',5);
end
...
function intParams = getInternalParams()
    ...
    intParams = struct('numPoints',100,'xRange',5,...
        ...
        'tRange',[0,1e3],'MaxWaveletPoints',1e4);
end
...
function status = plotTimeDomainDuring(t,y,flag,varargin)
    ...
    switch flag
        ...
        case 'init'
            ...
            %Initialise arrays
            T = NaN(1,TYInitSize);
            Y = NaN(3,TYInitSize);%NaN(TYInitSize,3);
            n = 1;
...
%         disp('some error');
    ...
    writematrix([t, x, p, a2, v],'phasespace.csv');
...
%         disp('some error');
    ...
    subplot(3,2,6);
    plot(t(1:end-1),diff(energy)./diff(t));
